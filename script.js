...
function toggleDeveloperMode() {
    ...
    showNotification(
        'Режим разработчика', 
        developerMode ? 'Включен' : 'Выключен'
    );
...
function addDeveloperTools() {
    ...
    if (!document.querySelector('.taskbar-app[data-app="ssapCompiler"]')) {
        const devElement = document.createElement('div');
        devElement.className = 'taskbar-app';
        devElement.setAttribute('data-app', 'ssapCompiler');
        devElement.innerHTML = '<i class="fas fa-code"></i>';
        devElement.onclick = () => openApp('ssapCompiler');
        taskbarApps.appendChild(devElement);
    }
}

function openDebugTools() {
    showNotification('Инструменты отладки', 'Открытие панели разработчика...');
}

function startGame() {
    showNotification('Игра', 'Запуск игры...');
}

function showInstructions() {
    showNotification('Инструкции', 'Управление: Стрелки для движения, Пробел для выстрела');
}
...
function completeSetup() {
    ...
    setTimeout(() => {
        showNotification('Добро пожаловать!', `Добро пожаловать в Soiav 2, ${username}!`);
    }, 1000);
}
...
function createLiveTiles() {
    ...
    const tiles = [
        ...
        { icon: 'fas fa-envelope', title: 'Почта', color: '#0078d7', size: 'small' },
        { icon: 'fas fa-map', title: 'Карты', color: '#107c10', size: 'small' },
        { icon: 'fas fa-cloud-sun', title: 'Погода', color: '#0078d7', size: 'small' }
    ];
    ...
    tiles.forEach(tile => {
        ...
        tileElement.onclick = () => {
            ...
            else if (tile.title === 'Почта') openApp('mail');
            else if (tile.title === 'Карты') openApp('maps');
            else if (tile.title === 'Погода') openApp('weather');
            toggleStartMenu();
        };
        ...
        case 'neofetch':
            response.innerHTML = `
                <div class="system-info">
                    <div class="ascii-art">
                        <pre>Soiav OS 2.0</pre>
                    </div>
                    <div class="sys-info">
                        <div>Soiav OS 2.0</div>
...
function logout() {
    if (confirm('Вы уверены, что хотите выйти?')) {
        showNotification('Системa', 'Выход из системы...');
        setTimeout(() => {
...
function createNewNote() {
    showNotification('Заметки', 'Создана новая заметка');
}

function openNote(noteId) {
    showNotification('Заметки', `Открыта заметка #${noteId}`);
}
